...
/*   Updated: 2023/01/18 16:18:00 by rbroque          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_map(t_data *data, const t_map *map)
{
	...
	while (i < map->y_size)
	{
		...
		{
			if (j < map->x_size - 1)
				put_line(data, &map->vcpy[i][j], &map->vcpy[i][j + 1], map->color_offset);
			if (i < map->y_size - 1)
				put_line(data, &map->vcpy[i][j], &map->vcpy[i + 1][j], map->color_offset);
			++j;
		}
		++i;
	}
